# Toonflow 接入与出片流程（实测记录，2026-09-17）

> 目标：让 **my_agent 通过 REST 驱动本机 Toonflow**（`127.0.0.1:10588`）走完
> 「原文 → 事件图谱 → 剧本 → 分镜 → 出图 → 出片」，产出**真动态**短视频。
> 本文记录实测通过的配置与调用序列，避免重复踩坑。

## 一、供应商与密钥（"改成我的 API"）

- Toonflow 的密钥存在它自己的库里（`data/db2.sqlite` → `o_vendorConfig`），
  实例：`id=agnes`，`inputValues.apiKey`、`baseUrl=https://api.agnes-ai.cn/v1`，`enable=1`。
- 与 my_agent 的 `IMAGE_GEN_API_KEY` / `VIDEO_GEN_API_KEY` 是**同一把 agnes key**（已逐字符比对一致）。
...
Toonflow 的每个 Agent 槽位在 `o_agentDeploy` 里，`resolveModelName()` 取的是
**`modelName` 字段**，并且要求它是 **`供应商:模型` 的完整串**：

```
key=universalAi      model=agnes-3.0-flash   modelName=agnes:agnes-3.0-flash   vendorId=agnes
key=scriptAgent      model=agnes-3.0-flash   modelName=agnes:agnes-3.0-flash   vendorId=agnes
key=productionAgent  model=agnes-3.0-flash   modelName=agnes:agnes-3.0-flash   vendorId=agnes
```
...
| 提示词生成失败：`未找到供应商配置 id=agnes-3.0-flash` | `modelName` 少了供应商前缀 → 代码按 `:` 切分后把模型名当成了供应商 id |

修复（官方 API，逐行更新）：

```
POST /api/setting/agentDeploy/updateAgentModel
{"id":3,"name":"通用AI","model":"agnes-3.0-flash",
 "modelName":"agnes:agnes-3.0-flash","vendorId":"agnes","desc":"…","temperature":0.7}
```
...
5. toonflow(command="gen_prompts", project_id=…,                    # 生成 [Visual]+[Motion] 提示词
             ...
             model="agnes:agnes-video-2.5-flash", mode="text")
6. toonflow(command="generate_video", project_id=…, script_id=…,     # 提交出片（**消耗视频额度**）
             ...
             model="agnes:agnes-video-2.5-flash", mode="text", resolution="720P")
...
- **时长**：`agnes-video-2.5-flash` 只支持 `mode=text`（**没有图生视频**），
...
```
项目: 第七次葬礼（Agnes 试跑）  projectId=1789640003887  scriptId=1
...
- Toonflow 的读接口**几乎全是 POST + JSON body**（1.1.8 版 169 条路由里 159 条 POST），
  用 GET 会 404；`toonflow(command="call", path=…)` 会按内置路由表自动选方法。
- 直接改 `data/db2.sqlite` 有缓存/并发风险，**优先走它的 HTTP API**。
- 服务日志与数据都在 `D:\aaa\work\work\Toonflow-app\data\`。
